...
export default function ItemModal({ mode, initial, onClose, onSave, duplicateCheck }) {
  ...
  const handleFile = async (file) => {
    ...
    setUploading(true);
    try {
      set("cover", await resizeImage(file));
    } catch (e) {
      console.error(e);
    } finally {
      setUploading(false);
  ...
  const submit = async (e) => {
    ...
    setError(null);
    try {
      await onSave({ ...form, tags, recommendedTracks });
    } catch (err) {
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, background: "#20180fcc" }}
    >
      <div
      ...
      >
        ...
        <form onSubmit={submit} className="col gap-4" style={{ padding: 24 }}>
          {!isWishlist && (
            <div className="row gap-4" style={{ alignItems: "flex-start" }}>
              <div
                ...
                style={{
                  width: 88, height: 88, background: C.panel, cursor: "pointer", flexShrink: 0,
              ...
              >
                {!form.cover && (
                  <div style={{ textAlign: "center" }}>
                    ...
                    <span style={{ fontSize: 10, color: C.muted }}>{uploading ? "..." : "cover"}</span>
                  </div>
                )}
              </div>
              <input
              ...
              <div style={{ fontSize: 12, color: C.muted, paddingTop: 8 }}>
                Add a photo of the cover or sleeve.
                {form.cover && (
                  <button type="button" className="btn-text" style={{ display: "block", marginTop: 4, textDecoration: "underline" }} onClick={() => set("cover", null)}>
                    remove photo
                  </button>
                )}
              </div>
          ...
          <div className="grid-auto" style={{ gridTemplateColumns: "1fr 1fr" }}>
            ...
            <Field label="Artist">
              <input className="input" value={form.artist} onChange={(e) => set("artist", e.target.value)} required />
            </Field>
          </div>
          ...
          <Field label="Genres / tags (comma separated)">
            <input
              className="input"
              value={typeof form.tags === "string" ? form.tags : form.tags.join(", ")}
              onChange={(e) => set("tags", e.target.value)}
              placeholder="post-punk, live, japan pressing"
            />
          </Field>
